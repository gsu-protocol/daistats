  ...
  return (
    <div className="column">
      <div className="box has-text-centered">
        <h3 className="title gsu-font" title={ilk.price}>${props.formater.format(ilk.price)}</h3>
        <p className="title subtitle is-size-4">{t('daistats.token_price', { token: props.token })}</p>
        {ilk.priceNxt > 0 && <p className="subtitle is-size-6 gsu-font">{t('daistats.next_osm_price')}: <span
        ...
        {ilk.priceMedian && <p className="title subtitle is-size-6 gsu-font" title={ilk.priceMedian}>Median: ${props.formater.format(ilk.priceMedian)}</p>}
      </div>
    </div>
  )
}
